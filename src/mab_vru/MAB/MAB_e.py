...
def plot_evolution(times, history, save_path=None):
    ...
    plt.title('ε-Greedy: Protocol Performance Evolution')
    plt.legend()
    plt.grid(True)
...
def compare_protocols(df: pd.DataFrame, epsilon: float = 0.1, save_path: Optional[Path] = None) -> Tuple[str, float]:
    """Compare V2V and V2I protocols using ε-greedy strategy."""
    logger.info("Starting ε-greedy comparison")
    ...
    for t in sorted(df['Time'].unique()):
        ...
        # Epsilon-greedy emphasizes minimizing delay more
        max_delay = max(v2v_delay, v2i_delay)
        max_loss = max(v2v_loss, v2i_loss)
        max_load = max(v2v_load, v2i_load)
        
        # Different weights: 70% delay, 20% loss, 10% load
        v2v_score = (
            0.7 * (v2v_delay / max_delay if max_delay > 0 else 0.0) +
            0.2 * (v2v_loss / max_loss if max_loss > 0 else 0.0) +
            0.1 * (v2v_load / max_load if max_load > 0 else 0.0)
        )
        
        v2i_score = (
            0.7 * (v2i_delay / max_delay if max_delay > 0 else 0.0) +
            0.2 * (v2i_loss / max_loss if max_loss > 0 else 0.0) +
            0.1 * (v2i_load / max_load if max_load > 0 else 0.0)
        )
        ...
        mab.update(0, v2v_reward)  # Arm 0 = V2V
        mab.update(1, v2i_reward)  # Arm 1 = V2I
